...
def main_(args: argparse.Namespace):
    ...
    image_pub = node.create_publisher(Image, '/zed/left/image_bgra', image_qos)
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            try:
                img_msg = bridge.cv2_to_imgmsg(image_left_ocv, encoding='bgra8')
                img_msg.header.stamp = node.get_clock().now().to_msg()
                img_msg.header.frame_id = 'zed2_left_camera_frame'
                image_pub.publish(img_msg)
            except Exception as e:
                node.get_logger().error(f"Image publish failed: {e}")
