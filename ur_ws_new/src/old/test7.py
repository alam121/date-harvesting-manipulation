...
class UR10eCuroboMoveIt(Node):
    ...
    def wait_for_key_press(self):
        """Wait for user input: 'y' to store goal, 'n' to execute, 'q' to quit."""
        print("Press 'y' to store a marker position. Press 'n' to execute. Press 'q' to exit.")
        ...
        while self.running:
            ...
            if key == "y" and self.latest_marker_pose:
                ...
                print(f"Goal {len(self.goal_poses)} saved: {goal_position}")
                
                # Publish goal marker in RViz
                self.publish_goal_marker(goal_position[:3])

            elif key == "n" and self.goal_poses:
                # Start execution only when 'n' is pressed
                ...
                self.get_logger().info("Skipping gripper command.")
                #break
            elif key == "q":
                print("Exiting...")
                self.running = False
    ...
    def control_gripper(self, action):
        """Call the ROS2 service to open or close the Robotiq gripper."""
        self.get_logger().info(f"Sending gripper command: {action}")

        client = self.create_client(RobotiqGripper, "/Robotiq_Gripper")
        while not client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("Waiting for Robotiq Gripper service...")

        request = RobotiqGripper.Request()
        request.action = action

        future = client.call_async(request)
        rclpy.executors.MultiThreadedExecutor().spin_until_future_complete(future)

        if future.result():
            self.get_logger().info(f"Gripper action '{action}' executed successfully.")
        else:
            self.get_logger().warn("Failed to execute gripper action.")
