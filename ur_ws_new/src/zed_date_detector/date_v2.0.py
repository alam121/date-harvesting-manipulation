...
T_LIVOX_TO_ZED = np.array([
    [ 0.0, -1.0,  0.0,  0.02],  # X_cam = -Y_liv + tx
    [ 0.0,  0.0,  1.0,  0.00],  # Y_cam =  Z_liv + ty
    [ 1.0,  0.0,  0.0,  0.10],  # Z_cam =  X_liv + tz
    [ 0.0,  0.0,  0.0,  1.00]
], dtype=np.float32)

# If your ZED coordinate system is RIGHT_HANDED_Y_UP, set this True (v = -Y/Z*fy + cy).
# date_v1.8.py uses project_point_to_image() without the minus, so default is False.
LIVOX_Y_UP = False

livox_msg = None
livox_lock = Lock()

# Livox temporal registration buffer (WORLD frame)
livox_world_buffer = deque(maxlen=8)
LIVOX_BUFFER_SEC = 0.35

# Debug counters (overlay)
livox_pts_raw = 0
livox_pts_valid_cam = 0
livox_pts_registered = 0
livox_pts_projected = 0
...
# ============================================================
# Livox helpers (pose + sparse depth map)
# ============================================================
def pose_to_T_cam_to_world(pose: sl.Pose) -> np.ndarray:
    """Convert ZED pose to 4x4 T_cam_to_world."""
    ...
    try:
        ...
        T[:3, :3] = R
        T[:3, 3] = np.array([float(tr[0]), float(tr[1]), float(tr[2])], dtype=np.float32)
    except Exception:
        pass
    return T


def depth_at_px_from_sparse_dmap(dmap: np.ndarray, px: int, py: int, r: int = 12) -> float:
    """Nearest finite depth around (px,py) within radius r (min depth)."""
    if dmap is None:
        return np.nan
    h, w = dmap.shape[:2]
    ...
    if np.isfinite(v):
        return float(v)
    x1 = max(0, px - r); x2 = min(w, px + r + 1)
    y1 = max(0, py - r); y2 = min(h, py + r + 1)
    win = dmap[y1:y2, x1:x2]
    if np.isfinite(win).any():
        return float(np.nanmin(win))
    return np.nan
...
def main_(args: argparse.Namespace):
    ...
    global livox_msg, livox_pts_raw, livox_pts_valid_cam, livox_pts_registered, livox_pts_projected
    ...
    # --- Livox subscription (PointCloud2) ---
    if USE_LIVOX:
        livox_qos = QoSProfile(depth=1, reliability=ReliabilityPolicy.BEST_EFFORT)

        def livox_cb(msg: PointCloud2):
            global livox_msg
            with livox_lock:
                livox_msg = msg

        node.create_subscription(PointCloud2, LIVOX_TOPIC, livox_cb, livox_qos)
        node.get_logger().info(f"[LIVOX] Subscribed to {LIVOX_TOPIC}")
    ...
    def perception_loop():
        ...
        t_prev = time()
        # FIX #2: name must be last_viz_local (not last_viz_local_local)
        last_viz_local = 0.0
        ...
        while not exit_signal:
            ...
            # --------------------------------------------------
            # Livox → (registered) sparse depth map in ZED image (full resolution)
            # FIX #1: this whole block must be INSIDE perception_loop (indented)
            # --------------------------------------------------
            livox_pts_raw = livox_pts_valid_cam = livox_pts_registered = livox_pts_projected = 0
            depth_livox_map = None

            if USE_LIVOX:
                # ZED pose for temporal registration
                pose_status = zed.get_position(cam_w_pose, sl.REFERENCE_FRAME.WORLD)
                if pose_status == sl.POSITIONAL_TRACKING_STATE.OK:
                    T_cam_to_world = pose_to_T_cam_to_world(cam_w_pose)
                else:
                    T_cam_to_world = np.eye(4, dtype=np.float32)
                T_world_to_cam = np.linalg.inv(T_cam_to_world).astype(np.float32)
                ...
                if pc_msg is not None:
                    try:
                        ...
                        livox_pts_raw = len(pts_list)

                        if livox_pts_raw > 0:
                            points = np.asarray(pts_list, dtype=np.float32)
                            hpts = np.hstack([points, np.ones((points.shape[0], 1), dtype=np.float32)])
                            pts_cam = (T_LIVOX_TO_ZED @ hpts.T).T[:, :3]

                            zc = pts_cam[:, 2]
                            valid = (zc > 0.4) & (zc < 50.0) & np.isfinite(zc)
                            pts_cam = pts_cam[valid]
                            livox_pts_valid_cam = int(pts_cam.shape[0])

                            # Buffer in WORLD frame for temporal registration
                            if pts_cam.shape[0] > 0:
                                hcam = np.hstack([pts_cam, np.ones((pts_cam.shape[0], 1), dtype=np.float32)])
                                pts_world = (T_cam_to_world @ hcam.T).T[:, :3]
                                tnow = time()
                                livox_world_buffer.append((tnow, pts_world))
                                while livox_world_buffer and (tnow - livox_world_buffer[0][0] > LIVOX_BUFFER_SEC):
                                    livox_world_buffer.popleft()

                            # Stack buffered clouds and reproject into current CAM frame
                            if livox_world_buffer:
                                any_nonempty = any((pw is not None and len(pw) > 0) for (_, pw) in livox_world_buffer)
                                pts_world_all = (
                                    np.vstack([pw for (_, pw) in livox_world_buffer if pw is not None and len(pw) > 0])
                                    if any_nonempty else np.empty((0, 3), dtype=np.float32)
                                )
                                livox_pts_registered = int(pts_world_all.shape[0])

                                if pts_world_all.shape[0] > 0:
                                    hworld = np.hstack([pts_world_all, np.ones((pts_world_all.shape[0], 1), dtype=np.float32)])
                                    pts_cam_all = (T_world_to_cam @ hworld.T).T[:, :3]

                                    z = pts_cam_all[:, 2]
                                    valid2 = (z > 0.4) & (z < 50.0) & np.isfinite(z)
                                    pts_cam_all = pts_cam_all[valid2]

                                    if pts_cam_all.shape[0] > 0:
                                        z = pts_cam_all[:, 2]
                                        fx = intrinsics["fx"]; fy = intrinsics["fy"]
                                        cx0 = intrinsics["cx"]; cy0 = intrinsics["cy"]
                                        cam_w = camera_res.width
                                        cam_h = camera_res.height

                                        u = (pts_cam_all[:, 0] / z) * fx + cx0
                                        if LIVOX_Y_UP:
                                            v = (-pts_cam_all[:, 1] / z) * fy + cy0
                                        else:
                                            v = (pts_cam_all[:, 1] / z) * fy + cy0
                                        ...
                                        valid_uv = (u_i >= 0) & (u_i < cam_w) & (v_i >= 0) & (v_i < cam_h)
                                        u_i = u_i[valid_uv]
                                        v_i = v_i[valid_uv]
                                        z = z[valid_uv]
                                        livox_pts_projected = int(len(u_i))

                                        if livox_pts_projected > 0:
                                            flat = np.full((cam_w * cam_h,), np.inf, dtype=np.float32)
                                            idx_flat = v_i * cam_w + u_i
                                            np.minimum.at(flat, idx_flat, z.astype(np.float32))
                                            flat[~np.isfinite(flat)] = np.inf
                                            dmap = flat.reshape((cam_h, cam_w))
                                            dmap[dmap == np.inf] = np.nan
                                            depth_livox_map = dmap

                    except Exception as e:
                        print("[LIVOX] Exception while processing Livox cloud:", e)
            ...
            for o in objects.object_list:
                ...
                try:
                    ...
                    # Livox depth at bbox center (full-res pixel)
                    Zliv_center = np.nan
                    if depth_livox_map is not None:
                        cx_full = int(round((bb[0][0] + bb[2][0]) * 0.5))
                        cy_full = int(round((bb[0][1] + bb[2][1]) * 0.5))
                        Zliv_center = depth_at_px_from_sparse_dmap(depth_livox_map, cx_full, cy_full, r=12)
                    ...
                    try:
                        ...
                        obj_confidence = getattr(o, "confidence", 0.5) / 100.0
            ...
            # --------------------------------------------------
            now = time()
            if not SKIP_DRAW and (now - last_viz_local) >= 0.1:
                if SHOW_REJECTED:
                ...
                for i, t in enumerate(targets):
                    ...
                    Zliv_c = t.get("Zliv_center", np.nan)
                    if np.isfinite(Zliv_c):
                        cv2.putText(
                            image_left_ocv,
                            f"LIV_Z:{Zliv_c:.2f}",
                            (cx + 10, cy + 40),
                ...
                if USE_LIVOX:
                    cv2.putText(
                        image_left_ocv,
                        f"Livox proj:{livox_pts_projected} raw:{livox_pts_raw} valid:{livox_pts_valid_cam} reg:{livox_pts_registered}",
                        (12, 72),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.55,
                        (255, 255, 255, 255),
