...
tf_buffer = Buffer()
tf_listener = None  # will be initialized in main_ after node creation
...
def cam_m_to_mm_desired(p_cam):
    x, y, z = p_cam  # meters in ZED CAMERA frame
    return np.array([x*1000.0, -y*1000.0, -z*1000.0], dtype=np.float32)
...
def main_(args: argparse.Namespace):
    global image_net, exit_signal, run_signal, detections, loop_fps, tf_listener

    # ROS2 setup
    rclpy.init()
    node = rclpy.create_node('zed_date_detector_t')
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            # ---------- PRINT / OVERLAY BOTH COORDINATES ----------
            # A) ZED SDK object position (converted to CAMERA frame)
            # B) Old-style centroid pixel sample from XYZ image
            if objects.is_new and len(objects.object_list) > 0:
                print("\nDetections this frame:")
            with centroids_lock:
                centroid_list = list(centroids)

            # Make a quick spatial index from centroid pixel -> text overlay anchor
            # (just use bbox top-left for label placement)
            for o in objects.object_list:
                name = label_to_name(o, class_names)
                # A) ZED -> CAMERA conversion
                pw = np.array([float(o.position[0]), float(o.position[1]), float(o.position[2])], dtype=np.float32)
                pc = world_to_camera(pw, cam_w_pose)  # meters, CAMERA frame
                Xc, Yc, Zc = float(pc[0]), float(pc[1]), float(pc[2])
                Xd, Yd, Zd = cam_m_to_mm_desired((Xc, Yc, Zc))
                # draw bbox & ZED coords
                if len(o.bounding_box_2d) == 4:
                    x1 = int(o.bounding_box_2d[0][0] * image_scale[0])
                    y1 = int(o.bounding_box_2d[0][1] * image_scale[1])
                    txt_a = f"{name} ZED_CAM=({Xc:.2f},{Yc:.2f},{Zc:.2f})m"
                    cv2.putText(image_left_ocv, txt_a, (x1, max(0, y1 - 18)),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (50, 200, 255, 255), 1, cv2.LINE_AA)

                print(f"  id={o.id} class={name}  DESIRED(mm)=[{Xd:.3f}, {Yd:.3f}, {Zd:.3f}]")
            # B) Old-style: sample single pixel at mask centroid from XYZ
            ...
            for (cx, cy, cls_i, conf_i) in centroid_list:
                if 0 <= cx < Wxyz and 0 <= cy < Hxyz:
                    ...
                    Xd, Yd, Zd = cam_m_to_mm_desired((Xp, Yp, Zp))
                    if np.isfinite(Xp) and np.isfinite(Yp) and np.isfinite(Zp):
                        print(f"     mask-centroid[ DESIRED(mm)=[{Xd:.3f}, {Yd:.3f}, {Zd:.3f}]") 
                        ...
                        cv2.putText(image_left_ocv, f"({Xd:.1f},{Yd:.1f},{Zd:.1f})mm",
                            (sx + 6, sy - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255,255), 1, cv2.LINE_AA)
