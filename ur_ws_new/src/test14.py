...
class UR10eCuroboMoveIt(Node):
    def __init__(self):
        ...
        self.motion_gen_config = MotionGenConfig.load_from_robot_config(
            "ur10e.yml", interpolation_dt=0.004) #,trajopt_dt=0.05, num_trajopt_seeds=5)
    ...
    def move_to_home_position(self, wait_time=4.0):
        """Move the robot to a predefined joint configuration."""
        if self.current_joint_positions is None:
            self.get_logger().warn("Joint state missing; cannot move to home.")
            return

        self.get_logger().info("Moving to joint-space home position…")
        ...
        pt = JointTrajectoryPoint()
        pt.positions = self.home_joints
        # Optional: you can set velocities/efforts here too
        pt.time_from_start.sec = int(wait_time)
        pt.time_from_start.nanosec = int((wait_time % 1.0) * 1e9)

        traj.points = [pt]

        # Publish to your trajectory controller
        self.trajectory_pub.publish(traj)

        # Wait roughly for the motion to finish
        time.sleep(wait_time)
        self.get_logger().info("Home joint target should now be reached.")
