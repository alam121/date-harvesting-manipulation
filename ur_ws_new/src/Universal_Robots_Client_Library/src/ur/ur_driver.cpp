...
void UrDriver::init(const UrDriverConfiguration& config)
{
  ...
  // Raised from 1000ms: the UR primary interface deprioritizes the one-time
  // ConfigurationData package when several clients (dashboard_client,
  // robot_state_helper, control node) connect at once, which can push delivery
  // past 1s and abort bring-up. 5s absorbs that contention regardless of ordering.
