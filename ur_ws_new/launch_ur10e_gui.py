...
class LaunchDialog(QtWidgets.QDialog):
    ...
    def start_system(self):
        if not LAUNCHER.exists():
            QtWidgets.QMessageBox.critical(self, "Launcher Missing", f"Could not find {LAUNCHER}")
            return

        args = self.build_args()
        if not args:
            QtWidgets.QMessageBox.warning(
                self, "Nothing Selected", "Select at least one node or preset before launching."
            )
            return
