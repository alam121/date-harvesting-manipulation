...
void DeltoExternalDriver::timer_callback()
{
	...
	RCLCPP_INFO(this->get_logger(), "Received Data: current.size() = %ld, joint.size() = %ld",
		    recieved_data.current.size(), recieved_data.joint.size());

	if (recieved_data.current.size() != 12 || recieved_data.joint.size() != 12)
	{
	    RCLCPP_ERROR(this->get_logger(), "Received data error! Current Size: %ld, Joint Size: %ld",
		         recieved_data.current.size(), recieved_data.joint.size());
	    return;
}
    ...
    if (joint_publish_count++ > 5)
    {
        ...
        joint_state.effort = electric_current_state;
